                                    ...
                                    @foreach ($cartItems as $key=>$item)
                                        <tr>
                                            <td class="product-thumbnail">
                                                <img src="{{ asset('students/' . $item->attributes->image) }}"
                                            ...
                                            <td>
                                                <form action="{{ route('cart.remove') }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" value="{{ $item->id }}" name="id">
                                                    <button type="submit" class="px-4 py-2 text-white bg-red-600"
                                                        style="border-radius: 7px; background-color: #ee4266; border-color: #ee4266;">x</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                            ...
                            <div class="col-md-7">
                                <div>
                                    <form action="{{ route('cart.clear') }}" method="POST">
                                        @csrf
                                        <button class="px-6 py-2 text-red-800 bg-red-300"
                                            style="margin-left: 3rem; border-radius: 7px; color: white; background-color: #ee4266; width: 15rem; border-color: #ee4266;">Remove
                                            All Cart</button>
                                    </form>
                                </div>
                                <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <button class="btn btn-primary btn-lg btn-block"
                                            onclick="window.location='{{ route('checkout.page') }}'">Proceed To
                                            Checkout</button>
                                    </div>
                                </div>
                            </div>
